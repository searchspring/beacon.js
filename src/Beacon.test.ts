...
describe('Beacon', () => {
	...
	describe('Globals', () => {
		...
		it('can set current cart from globals', async () => {
			...
			expect(beacon.storage.cart.get()).toEqual(cart);
			...
			await new Promise((resolve) => setTimeout(resolve, 0));
			expect(beacon.storage.cart.get()).toEqual(cart2);
			expect(mockFetchApi).toHaveBeenCalledWith(expect.stringContaining('/cart/add'), expect.any(Object));
			expect(mockFetchApi).toHaveBeenCalledWith(expect.stringContaining('/preflightCache'), expect.any(Object));
			mockFetchApi.mockClear();
			expect(mockFetchApi).not.toHaveBeenCalled();
			...
			await new Promise((resolve) => setTimeout(resolve, 0));
			expect(beacon.storage.cart.get()).toEqual(cart3);
			expect(mockFetchApi).toHaveBeenCalledWith(expect.stringContaining('/cart/add'), expect.any(Object));
			mockFetchApi.mockClear();
			expect(mockFetchApi).not.toHaveBeenCalled();
			...
			await new Promise((resolve) => setTimeout(resolve, 0));
			expect(beacon.storage.cart.get()).toEqual(cart4);
			expect(mockFetchApi).toHaveBeenCalledWith(expect.stringContaining('/cart/remove'), expect.any(Object));
			mockFetchApi.mockClear();
			expect(mockFetchApi).not.toHaveBeenCalled();

			// simulate adding to cart without quantity and price and bare minimum product object
			const cart5 = [
				{ uid: 'productUid3', sku: 'productSku3' },
			] as any;
			beacon = new Beacon({
				...mockGlobals,
				cart: cart5,
			...
			await new Promise((resolve) => setTimeout(resolve, 0));
			expect(beacon.storage.cart.get()).toEqual([{
				...cart5[0],
				price: -1,
				qty: 1,
			}]);
			...
			await new Promise((resolve) => setTimeout(resolve, 0));
			expect(beacon.storage.cart.get()).toEqual(cart6);
			expect(mockFetchApi).toHaveBeenCalledWith(expect.stringContaining('/cart/remove'), expect.any(Object));
			mockFetchApi.mockClear();
			expect(mockFetchApi).not.toHaveBeenCalled();
			...
			await new Promise((resolve) => setTimeout(resolve, 0));
			expect(beacon.storage.cart.get()).toEqual(cart7);
			expect(mockFetchApi).toHaveBeenCalledWith(expect.stringContaining('/cart/remove'), expect.any(Object));
			mockFetchApi.mockClear();
			expect(mockFetchApi).not.toHaveBeenCalled();
		});
