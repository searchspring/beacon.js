...
export class Beacon {
	...
	constructor(globals: BeaconGlobals, config?: BeaconConfig) {
		...
		const fetchApi = this.config.fetchApi;
	...
	storage = {
		cart: {
			...
			clear: (): void => {
				...
				if (products.length) {
					this.setCookie(CART_KEY, '', COOKIE_SAMESITE, EXPIRED_COOKIE, COOKIE_DOMAIN); // remove cookie by setting past expiration
	...
	public sendPreflight(overrides?: { userId: string; siteId: string; shopper: string; cart: Product[]; lastViewed: string[] }): void {
		...
		if (userId && typeof userId == 'string' && siteId && (shopper || cart.length || lastViewed.length)) {
			...
			if (typeof fetch !== 'undefined' || this.config.fetchApi) {
				(this.config.fetchApi || fetch)(endpoint, {
					method: 'POST',
					headers: {
